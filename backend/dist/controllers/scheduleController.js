...
exports.getSchedulingAnalytics = exports.getRescheduleSuggestions = exports.getScheduleHistory = exports.approveBatchSchedules = exports.validateBatchSchedules = exports.autoGenerateSchedules = exports.getAIScheduleRecommendation = exports.deleteSchedule = exports.updateSchedule = exports.createSchedule = exports.getScheduleById = exports.getAllSchedules = exports.scheduleCreateSchema = void 0;
...
async function runBatchValidation(proposed, existing, buses, drivers, routes, maintenances) {
    const conflicts = [];
    ...
    for (let i = 0; i < proposed.length; i++) {
        ...
        if (isBusInMaintDb) {
            const altBus = buses.find(b => b.id !== p.busId &&
                b.status !== 'INACTIVE' &&
        ...
        if (isDriverInactive) {
            const altDriver = drivers.find(d => d.id !== p.driverId &&
        ...
        const otherPropBusOverlap = proposed.find((pr, idx) => idx !== i &&
            pr.busId === p.busId &&
            hasOverlap(pStart, pEnd, timeToMinutes(pr.departureTime), timeToMinutes(pr.arrivalTime)));
        const dbBusOverlap = existing.find(s => s.id !== p.id &&
            s.busId === p.busId &&
        ...
        if (otherPropBusOverlap || dbBusOverlap) {
            ...
            const altBus = buses.find(b => b.status !== 'INACTIVE' &&
                b.status !== 'MAINTENANCE' &&
                !maintenanceBusIds.has(b.id) &&
                !existing.some(s => s.busId === b.id && hasOverlap(pStart, pEnd, timeToMinutes(s.departureTime), timeToMinutes(s.arrivalTime))) &&
        ...
        const otherPropDriverOverlap = proposed.find((pr, idx) => idx !== i &&
            pr.driverId === p.driverId &&
            hasOverlap(pStart, pEnd, timeToMinutes(pr.departureTime), timeToMinutes(pr.arrivalTime)));
        const dbDriverOverlap = existing.find(s => s.id !== p.id &&
            s.driverId === p.driverId &&
            hasOverlap(pStart, pEnd, timeToMinutes(s.departureTime), timeToMinutes(s.arrivalTime)));
        if (otherPropDriverOverlap || dbDriverOverlap) {
            const overlapWith = otherPropDriverOverlap || dbDriverOverlap;
            const altDriver = drivers.find(d => d.availabilityStatus !== 'OFF_DUTY' &&
                d.availabilityStatus !== 'SUSPENDED' &&
                !existing.some(s => s.driverId === d.id && hasOverlap(pStart, pEnd, timeToMinutes(s.departureTime), timeToMinutes(s.arrivalTime))) &&
        ...
        if (driver && !isDriverInactive && !otherPropDriverOverlap && !dbDriverOverlap) {
            ...
            for (const s of driverSchedules) {
                if (s.departureTime === p.departureTime)
                    continue;
...
const autoGenerateSchedules = async (req, res, next) => {
    try {
        ...
        const existingSchedules = await prisma_1.default.schedule.findMany({
            where: { status: 'ACTIVE' },
            include: {
                route: true,
                bus: true,
        ...
        const getDriverDailyDuration = (driverId, proposed) => {
        ...
        const getDriverAllSchedulesForDay = (driverId, proposed) => {
...
exports.autoGenerateSchedules = autoGenerateSchedules;
// 2. Validate Batch (POST)
const validateBatchSchedules = async (req, res, next) => {
    try {
        const { schedules } = req.body;
        if (!schedules || !Array.isArray(schedules)) {
            throw new error_1.ApiError(400, 'Invalid schedules list provided');
        }
        const buses = await prisma_1.default.bus.findMany();
        const drivers = await prisma_1.default.driver.findMany({ include: { user: true } });
        const routes = await prisma_1.default.route.findMany();
        const maintenances = await prisma_1.default.maintenance.findMany({
            where: { status: { in: ['SCHEDULED', 'IN_PROGRESS'] } }
        });
        const existing = await prisma_1.default.schedule.findMany({
            where: { status: 'ACTIVE' },
            include: {
                route: true,
                bus: true,
...
exports.validateBatchSchedules = validateBatchSchedules;
// 3. Approve Batch (POST)
const approveBatchSchedules = async (req, res, next) => {
    try {
        const { schedules, conflictsResolved } = req.body;
        if (!schedules || !Array.isArray(schedules)) {
            throw new error_1.ApiError(400, 'Invalid schedules list provided');
        }
        const createdSchedules = await prisma_1.default.$transaction(async (tx) => {
            const results = [];
            for (const s of schedules) {
                const route = await tx.route.findUnique({ where: { id: s.routeId } });
                if (!route) {
                    throw new error_1.ApiError(400, `Route ID ${s.routeId} does not exist`);
                }
        ...
        if (req.user?.id) {
            const dbUser = await prisma_1.default.user.findUnique({ where: { id: req.user.id } });
            if (dbUser) {
...
exports.approveBatchSchedules = approveBatchSchedules;
...
const getRescheduleSuggestions = async (req, res, next) => {
    try {
        const activeSchedules = await prisma_1.default.schedule.findMany({
            where: { status: 'ACTIVE' },
            include: {
                route: true,
                bus: true,
            ...
            if (isBusMaint || isBusInactive || isDriverInactive) {
                ...
                if (isDriverInactive) {
                    ...
                    const alternatives = activeDrivers.filter(d => {
                        ...
                        if (dailyDur + s.route.totalDuration > 480)
                            return false;
                        return true;
                    });
...
exports.getRescheduleSuggestions = getRescheduleSuggestions;
// 6. Get Scheduling Analytics (GET)
const getSchedulingAnalytics = async (req, res, next) => {
    try {
        const buses = await prisma_1.default.bus.findMany();
        const drivers = await prisma_1.default.driver.findMany({ include: { user: true } });
        const routes = await prisma_1.default.route.findMany();
...
exports.getSchedulingAnalytics = getSchedulingAnalytics;
